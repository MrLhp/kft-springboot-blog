package com.kafeitoo.blog.util;

import cn.hutool.json.JSONUtil;
import com.kafeitoo.blog.bean.EqualAmountBean;
...
public class EqualAmountOfInterest {
     ...
     */
    public List<EqualAmountBean> execEqualAmountOfInterest(double quota, double interestRate, double interestRebate, Date dateOfLoan, int stageNumberOfMonth) {
        ...
        //折扣
        if (interestRebate >0) {
            _interestRate = _interestRate.multiply(BigDecimal.valueOf(interestRebate)).setScale(2, RoundingMode.HALF_UP);
        }
        ...
        for (int i = 1; i <= stageNumberOfMonth; i++) {
            ...
            _interestRate = _quota.multiply(BigDecimal.valueOf(days)).multiply(BigDecimal.valueOf(interestRate)).setScale(2,RoundingMode.HALF_UP);
    ...
    @Test
    public void test() {
        List<EqualAmountBean> equalAmountBeans = this.execEqualAmountOfInterest(10000, 0.0004, 0, new Date(), 12);
        for (EqualAmountBean equalAmountBean : equalAmountBeans) {
            System.out.println(JSONUtil.parse(equalAmountBean));
        }
    }
